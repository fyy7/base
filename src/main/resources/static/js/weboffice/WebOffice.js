...
function WebOffice2015() {
            ...
            if (httpclient.Send()) 
            {
            	if (this.hiddenSaveLocal(httpclient, this, false, false)) {
					...
					if (mSaveResult == 0) { // 打开本地磁盘文件
						...
						var windows = window.confirm("打开文档时未知错误错误码为： "
										+ mSaveResult
										+ "\r\r单击“确定”关闭。单击“取消”继续。");
						this.Status = "打开文档时未知错误错误码为： "
								+ mSaveResult;
						if (windows == 1) {
							window.close();
							return false;
	...
	this.WebOpen3 = function(downloadLink)
	{
		...
		var result = httpclient.Open(this.HttpMethod.Get, downloadLink, false);
		if (result)
        {
            if (httpclient.Send()) 
            {
	...
	/* 从服务器上取文档并打开，打开RecordID指定的文件 */   //该功能已完整实现
	this.WebOpenBase64 = function(uid,istemp,callback)
	{
	    ...
	    this.ShowMenuBar(this.ShowMenu);  //控制菜单栏是否可以显示
		this.NewShowToolBar(this.ShowToolBar); //控制Office工具栏和自定义工具栏
	    var url = this.ServerUrl + "&KIND_WEBOFFICE_OPTION=LOADBASE64FILE&UID="+uid + "&IS_TMP_FILE=" + istemp;
		$.ajax({
			  type: 'POST',
			  url: url,
	          cache: false,
	          ...
	          xhr: function(){ //这是关键 获取原生的xhr对象 做以前做的所有事情
		        	...
		        	//console.log(xhr);//这里输出的是request信息
			  ...
			  success: function(data) {
				  ...
				  var base64data = jsondata.content;
				  if(jsondata.g_result=='1'){
					  var filePathName = tmpDownPath;
			    	  filePathName += "\\";
			    	  filePathName += filename;
			    	  officeobj.Base64Text2File(base64data, filePathName);
			    	  jsWebOffice.WebOpenLocalFile(filePathName);
				  }
		    	  
		    	  delete jsondata.content;
		    	  jsondata.g_message = "加载成功";
		    	  this.Status = jsondata.g_message;
		    	  callback(jsondata);
	          },
	          error: function(XMLHttpRequest, textStatus, errorThrown)
	          {
	        	  this.Status = '打开失败，代码：'+textStatus + '，请联系管理员';
				  callback({g_result:0,g_message:this.Status});
	          }
	...
	this.WebSave2 = function(data, callback)
	{
		...
		if (!(mSaveResult == 0)) {
			this.Status = "保存本地文档失败错误代码为：" + mSaveResult;
			return false;
		}else{
			//alert("文档没有内容，是否确定保存");
		}
        ...
        var filedataObj = base64toBlob(base64file);
        formData.append("FileData", filedataObj, "upfilename");
        if(data){
        	var paramsKey = Object.keys(data);
        	for(j = 0,len=paramsKey.length; j < len; j++) {
        		formData.append(paramsKey[j],data[paramsKey[j]]);
        	}
        }
        
        //MultiPartUpload
        var strUrl = this.ServerUrl + '&KIND_WEBOFFICE_OPTION=MULTIPARTUPLOAD';
        $.ajax({
        	url: strUrl,
            type: "POST",
            data: formData,
...
//	   	}; 
	    ...
	    xmlhttp.open("POST",jsWebOffice.WebUrl + '/kind-oa/do?action=save_weboffice', false);   // false 是同步
    ...
    this.WebUseTemplate = function(uid,istemp)
    {
    	...
    	var httpclient = this.obj.Http; //设置http对象
    	/*
    	this.WebSetMsgByName("OPTION", "LOADFILE");
    	this.WebSetMsgByName("UID", uid);
    	this.WebSetMsgByName("IS_TMP_FILE", istemp);
    	this.WebSetMsgByName("TEMPLATE", this.Template);
    	httpclient.AddForm("FormData", this.GetMessageString()); //这里是自定义json 传输格式。
    	*/
    	httpclient.AddForm("UID", uid);
    	httpclient.AddForm("KIND_WEBOFFICE_OPTION", "LOADFILE");
    	httpclient.AddForm("IS_TMP_FILE", istemp);
    	this.WebClearMessage(); //清除所有WebSetMsgByName参数
...
/*	 文档保存到本地（有对话框）
	...
	this.VBAInsertFile = function(Position, FileName) 
	{
		try {
			...
			if (docType == this.DocType.WORD) {
				this.obj.Activate(true);
				console.log(this.obj.ActiveDocument.BookMarks.Exists(Position));
				if (this.obj.ActiveDocument.BookMarks.Exists(Position)) {
					//var wobj = this.obj;
					//setTimeout(function(){wobj.ActiveDocument.Application.Selection.GoTo(-1, 0, 0, Position)},200);
					var range = this.obj.ActiveDocument.Bookmarks.Item(Position).Range;
					range.Select();
				}
				this.obj.Activate(true);
    ...
    */
	...
	this.SAVEFILE = function(httpclient, FileName) // 与后台发生交互OPTION值为：SAVEFILE
	{
		...
		{
			if (!httpclient.Send()) {
				//this.Status = "数据包发送失败请检查链接<" + this.WebUrl + ">是否正确或网络是否畅通。";
			...
			//this.Status = "打开链接<" + this.WebUrl + ">失败请检查网络是否畅通。";
			this.Status = httpclient.GetResponseHeader("STATUS");
			return false;
		}
	}
	...
	this.LOADFILE = function(httpclient) 
	{
		this.Status = "";
		httpclient.ShowProgressUI = this.ShowWindow;
		this.WebUrl = this.domain+this.ServerUrl;
		if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) // true 异步方式 false同步
		{
			if (httpclient.Send()) {
				...
				if (this.hiddenSaveLocal(httpclient, this, false, false)) {
					...
					if (mSaveResult == 0) { // 打开本地磁盘文件
						...
						this.obj.FuncExtModule.Alert("打开文档时未知错误错误码为： " + mSaveResult);
						this.Status = "打开文档时未知错误错误码为： " + mSaveResult;
						window.close();
						return false;
					}
				} else {
